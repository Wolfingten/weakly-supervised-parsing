...
class SelfTrainingClassifier:
    def __init__(
        self, inside_model, num_iterations: int = 5, prob_threshold: float = 0.99
    ):
    ...
    def fit(
        ...
        self.inside_model.load_model(
            ...
            + f"inside_model_self_trained_base.onnx"
        )

        unlabeledy = self.inside_model.predict(
            pd.DataFrame(dict(sentence=unlabeled_inside_strings)),
            scale_axis,
            predict_batch_size,
        )
        unlabeledprob = self.inside_model.predict_proba(
            pd.DataFrame(dict(sentence=unlabeled_inside_strings)),
            scale_axis,
            predict_batch_size,
        )
        ...
        idx = 0
        while (
            len(unlabeledy_old) == 0 or np.any(unlabeledy != unlabeledy_old)
        ) and idx < self.num_iterations:
            unlabeledy_old = np.copy(unlabeledy)
            uidx = np.where(
                (unlabeledprob[:, 0] > self.prob_threshold)
            ...
            train_df_concat = pd.DataFrame(
                dict(
                    sentence=np.concatenate(
                        (labeled_inside_strings, unlabeled_inside_strings[uidx])
                    ),
            ...
            self.inside_model.load_model(
                pre_trained_model_path=TRAINED_MODEL_PATH + f"{filename}.onnx"
            )
